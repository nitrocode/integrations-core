...
def ci(ctx: click.Context, sync: bool):
    ...
    for data in construct_job_matrix(app.repo.path, get_all_targets(app.repo.path)):
        only_supports_python2 = bool(data.get('py2', False))
        config = {
            'job-name': data['name'],
            'target': data['target'],
            ...
            'test-py2': True if only_supports_python2 else '${{ inputs.test-py2 }}',
            'test-py3': False if only_supports_python2 else '${{ inputs.test-py3 }}',
    ...
    for field in ('repo', 'python-version', 'setup-env-vars'):
